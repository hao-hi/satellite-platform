...
3. `satmodel-platform-ui`：中文本地浏览器控制台，支持发现 `scenarios/` 下的场景和实验计划、查看和校验场景、从场景生成新实验计划、校验计划、运行实验，并在控制台内嵌浏览结果摘要、关键 run 对比、本地三维姿态回放、任务模式时间线与运行时调度联动，以及 dashboard 预览。
