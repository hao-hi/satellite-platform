...
第 `i` 个反作用轮由 spin axis `a_i`、spin inertia `J_i`、轮速 `Omega_i` 和电机力矩 `u_i` 描述：
...
CubeSat plant 中的 `ReactionWheelStateEffector` 负责轮组分配和受限电机力矩，再让轮速状态与刚体状态在同一步 RK4 中传播。公开的独立轮组离散传播路径已删除，避免同一轮组 plant 有两套状态演化口径。
...
这也是本轮新增 telemetry 中 `wheel_momentum_capacity_nms` 的物理含义。
...
其中 `z` 位于轮组 null space。`nullspace_momentum` 模式使用这个自由度做内部动量管理。它构造目标电机力矩：

$$
u_{ref,i}=k_hJ_i(\Omega_{ref,i}-\Omega_i)
$$

然后在满足 `A u=-tau_cmd` 和本步可行窗口的前提下，让实际 `u` 尽量靠近 `u_ref`。对三轮正交这类无冗余构型，null-space 维度为零，行为会自动退化为普通有界分配。
...
未饱和且轮轴满秩时该残差应接近零；力矩饱和、轮速饱和或故障后它描述命令不可实现程度。telemetry 还会记录本步可用力矩窗口、自由轮 mask、启用轮轴 rank、请求力矩和可实现力矩。
...
这是一个冗余四轮基线，轮组可通过有界分配产生三轴力矩。wheel failure 会把禁用轮命令和实际力矩置零，并对剩余启用轮重新计算 rank。若剩余三轮仍满秩，四轮金字塔可退化为 reduced-capability 三轴控制；若 rank 低于三，轮组返回最接近可实现力矩并报告非零分配残差。
...
| 轮组 torque/momentum envelope | Markley et al., "Maximum Torque and Momentum Envelopes for Reaction Wheel Arrays" | 后续约束分配和容量包络依据 |
| 轮系不确定和鲁棒控制背景 | Chen and Hu 2023，Hasan et al. 2022 | 后续控制研究锚点 |
| 四轮分配的更高级约束讨论 | Pyramid wheel allocation literature，例如 minimum infinity-norm 方向 | 后续，不在本轮 |
