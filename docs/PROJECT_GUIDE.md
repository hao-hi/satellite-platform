...
- 姿态 truth model 默认是刚体，不做柔性模态、燃料晃动和关节多体
- 气动和太阳光压默认使用盒体投影面积，不做 facet 遮挡、自阴影和复杂材料
- 太阳方向与地影仍是简化模型
- IGRF、NRLMSIS 和 TLE 只提供可选适配入口，默认安装不依赖外部地球模型包

这个取舍对应项目当前目标：先得到一个可组合、可追溯、便于替换的姿控 plant，再按任务需要升级物理复杂度。
...
| `physics.py` | 质量属性、1U CubeSat demo 物理配置 |
...
9. 把真值、估计、测量、力矩分项和轮组 telemetry 记录到 `SimulationResult`。
...
其余两轴循环替换即可。首版 CubeSat demo 还把四个轮的偏置质量贡献折成简化各向同性项：

$$
I_{demo} = I_{bus} + N_{rw}m_{rw}r_{off}^{2}I_3
$$

这是 demo 量级近似，不等同 CAD 质量属性。若已知每个部件的位置与自身惯量，应改用完整平行轴定理：
...
所以反作用轮不仅输出 `-Au`，其内部动量也进入刚体陀螺耦合项。这个结构比“控制器输出裁剪后直接塞给刚体”更接近真正 wheel plant。
...
| 轨道源 | 当前用途 |
| --- | --- |
| `CircularOrbitProvider` | 默认 LEO demo |
...
当前 CP 偏置、阻力系数和盒体几何都是可换配置。若研究不同面材料和迎风面变化，应升级为 facet/panel 求和模型。
...
重构后再做滤波与有界 RLS 更新。这个辨识器适合当前对角惯量 demo，不等同完整非对角惯量辨识或飞行在轨辨识方案。
...
| 参考项目参数 | 开源 demo 提供的首版参数 | 可跑通 baseline，后续应换成目标硬件数据 |
...
| 1U demo mass `2.6 kg` | CubeSat wheel demo 基线 | 参考项目参数 |
| 1U side length `0.1 m` | CubeSat wheel demo 基线 | 参考项目参数 |
| wheel mass `0.13 kg` | demo 惯量近似 | 参考项目参数 |
| wheel spin inertia `2.6e-5 kg m^2` | 轮组配置 | 参考项目推导值 |
| wheel max torque `0.007 N m` | 轮组配置 | 参考项目参数 |
| wheel max speed `8000 rpm` | 轮组配置 | 参考项目参数 |
...
| IGRF inputs | epoch and geodetic position | 官方模型输入口径 |
| NRLMSIS inputs | epoch, geodetic point, `F10.7`, `F10.7a`, `Ap` | 官方模型输入口径 |
...
| hub 加 effector 的结构思路 | Basilisk |
| 刚体/柔性/多体远期边界 | NASA 42；柔性多体相关论文 |
| 轨道环境与 panel/macromodel 升级路线 | Tudat |
| 任务层 spacecraft 资源划分 | GMAT |
| 四轮金字塔、轮限幅和故障 demo 基线 | `brunopinto900/attitude_control_reaction_wheels` |
...
| `examples/cubesat_wheel_failure.py` | 轮失效 smoke scenario |
...
1. 用任务级 `MassProperties` 替换 demo 惯量和 CP 偏置。
2. 若轨道对扰动变化敏感，切换到 ephemeris/TLE 输入，再评估是否需要独立数值 propagator。
3. 若气动是主要扰动，优先升级密度模型与姿态相关面积，再考虑 facet drag。
...
- Basilisk spacecraft/reaction-wheel documentation
- Tudat environment and spacecraft macromodel documentation
- NASA 42
- GMAT spacecraft attitude documentation
- NOAA IGRF
- NASA CCMC NRLMSIS
- Lee et al. reaction-wheel configuration study
- Markley et al. reaction-wheel torque and momentum envelopes
