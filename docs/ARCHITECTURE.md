...
`satmodel-platform-ui` 是当前轻量操作入口。它使用本地 HTTP 服务包装平台 API，发现 `scenarios/` 下的场景和实验计划，可以从场景生成新的 `ExperimentPlan`，并通过 `ExperimentRunner` 执行校验和运行；生成的结果仍然落在标准实验目录中，由 `dashboard.html` 负责展示。
